package com.cryptoalgo.oursql.support.ui;

import javafx.scene.paint.Color;

public class Colors {
    /**
     * A background color to be used for indicating a success
     */
    public static final Color SUCCESS = Color.rgb(0, 77, 55);
    /**
     * A background color to be used for indicating an in-progress operation
     */
    public static final Color LOADING = Color.rgb(102, 57, 6);
    /**
     * A background color to be used for indicating a failure or error
     */
    public static final Color ERROR = Color.rgb(102, 18, 19);
    /**
     * Background of splash screen
     */
    public static final Color SPLASH_BG = Color.rgb(30, 47, 32);
}
